...
describe('RegisterService', () => {
  ...
  it('should register a new user and mark that user as logged in', inject([RegisterService], (service: RegisterService) => {
    // TODO switch to a MockUser if time
    detail = {
    ...
    service
      .register(detail.email, detail.firstName, detail.lastName, detail.age, detail.location, detail.password, detail.confirmPassword)
      .subscribe((data) => {
  ...
  it('should return an error if the passwords do not match', inject([RegisterService], (service: RegisterService) => {
    ...
    service.register(detail.email, detail.firstName, detail.lastName, detail.age, detail.location, detail.password, detail.confirmPassword);
    expect(handleErrorSpy).toHaveBeenCalledWith('Passwords do not match');
    service.snackBar.dismiss(); // Dismiss at the end to unblock the view on Karma
  }));

  it('should return an error for existing email', inject([RegisterService], (service: RegisterService) => {
    ...
    service
      .register(detail.email, detail.firstName, detail.lastName, detail.age, detail.zip, detail.password, detail.confirmPassword)
      .subscribe(
        (data) => {},
  ...
  it('should return an error for invalid zip code', inject([RegisterService], (service: RegisterService) => {
    handleErrorSpy = spyOn<any>(service, 'handleError').and.callThrough();
    detail = {
      email: 'existingUser@gmail.com',
      first_name: 'Sloan',
  ...
  it('should return an error for an invalid age', inject([RegisterService], (service: RegisterService) => {
    handleErrorSpy = spyOn<any>(service, 'handleError').and.callThrough();
    detail = {
      email: 'existingUser@gmail.com',
      first_name: 'Sloan',
